% This is LNCSE.DEM the demonstration file of
...
%
\usepackage{multicol} % needed for the author index
...
%
\addtocmark[2]{Hamiltonian Mechanics} % additional numbered TOC mark
%
\title{Entire and Meromorphic Solutions of Ordinary Differential
Equations}
%
\titlerunning{Solutions of Ordinary Differential Equations}
% abbreviated title (for running head)
%
\author{Ivar Ekeland\inst{1} \and Roger
Temam\inst{2}
...
%\tocauthor{Ivar Ekeland {\rm (Universit\'{e} de Paris-Sud)},
%Jeffrey Dean, Dave Grove, Craig Chambers {\rm (Universit\`a di Geova)},
%Kim B. Bruce {\rm(Stanford University)},
%Elisa Bertino {\rm(Digita Research Center)}}
%
...
\index{Ekeland@Ivar Ekeland}
\index{Dean@Jeffrey Dean}
\index{Grove@David Grove}
\index{Chambers@Craig Chambers}
\index{Bruce@Kim B. Bruce}
\index{Bertino@Elisa Bertino}
\begin{abstract}
...
\end{table}
%
\begin{theorem} [Ghoussoub-Preiss]\label{ghou:pre}\quad\hskip-12pt
Assume $H(t,x)$ is
...
is called
$\left(A_{\infty} ,B_{\infty}\right)$-{\it sub\-qua\-dra\-tic at
in\-fi\-ni\-ty} if there exists a function $N(t,x)$ such that:
\begin{equation}
...
%
\title{Spectral Elements for Transport-Dominated Equations}
...
\end{table}
%
\begin{theorem} [Ghoussoub-Preiss]\quad\hskip-12pt
Assume $H(t,x)$ is
$(0,\varepsilon )$-subquadratic at
infinity for all $\varepsilon > 0$, and $T$-periodic in $t$
\begin{equation}
